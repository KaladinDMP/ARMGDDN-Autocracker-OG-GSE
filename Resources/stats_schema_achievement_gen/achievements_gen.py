        ...
        if s['type'] == 'int':
            try:
                ...
                try:
                    ...
                    print(f"Error: Cannot convert '{s['default']}' to an integer for stat '{s['name']}'. Setting default_num to 0.")
                    # Optionally, log the problematic stat for further review
                    with open("conversion_errors.log", 'a') as error_log:
                        error_log.write(f"Stat '{s['name']}': Cannot convert '{s['default']}' to int\n")
                    default_num = 0  # Set a default value or handle as needed
        else:
            try:
                default_num = float(s['default'])
            except ValueError:
                print(f"Error: Cannot convert '{s['default']}' to a float for stat '{s['name']}'. Setting default_num to 0.0.")
                # Optionally, log the problematic stat for further review
                with open("conversion_errors.log", 'a') as error_log:
                    error_log.write(f"Stat '{s['name']}': Cannot convert '{s['default']}' to float\n")
                default_num = 0.0  # Set a default value or handle as needed
